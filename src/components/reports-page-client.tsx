...
export function ReportsPageClient({
  ...
  return (
    <div>
      ...
      <SectionHeader eyebrow="analytics" title="Relatorios anuais">
        <YearFilter year={selectedYear} />
      </SectionHeader>

      <section className="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard title="Entradas no ano" value={data.metrics.annualIncome} caption="Inclui salarios mensais ativos" />
        <MetricCard title="Saidas no ano" value={data.metrics.annualExpense} tone="red" />
        <MetricCard title="Saldo anual" value={data.metrics.annualBalance} tone={data.metrics.annualBalance >= 0 ? "green" : "red"} />
        <MetricCard title="Media mensal de saidas" value={data.metrics.averageMonthlyExpense} tone="neutral" />
      </section>

      <section className="mb-6 grid gap-4 lg:grid-cols-2">
        <article className="glass-panel rounded-[1.7rem] p-5">
          <p className="text-sm text-[#96a59b]">Melhor mes</p>
          <h2 className="mt-2 text-2xl font-semibold text-[#39ff14]">{getMonthLabel(data.metrics.bestMonth.monthKey)}</h2>
          <p className="mt-1 text-sm text-[#96a59b]">Maior saldo do ano selecionado.</p>
        </article>
        <article className="glass-panel rounded-[1.7rem] p-5">
          <p className="text-sm text-[#96a59b]">Pior mes</p>
      ...
      <section className="mb-6 glass-panel rounded-[1.7rem] p-5">
        <h2 className="text-xl font-semibold text-white">Evolucao anual</h2>
      ...
      <section className="glass-panel rounded-[1.7rem] p-5">
        ...
        <p className="mt-1 text-sm text-[#96a59b]">Soma anual das saidas por categoria.</p>
