      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_0.8fr]">
        <section className="glass-panel rounded-[1.7rem] p-5">
          ...
          {data.accounts.length ? (
              ...
              {data.accounts.map((account) => (
                <article key={account.id} className="rounded-3xl border border-white/10 bg-white/[0.035] p-4">
                  <div className="mb-4 flex items-center justify-between gap-3">
                    ...
                    <span className="rounded-full border border-white/10 px-3 py-1 text-sm text-[#a7b4ac]">{getAccountTypeLabel(account.type)}</span>
                  </div>
                  <h3 className="text-lg font-semibold text-white">{account.name}</h3>
                  <p className="text-sm text-[#96a59b]">{account.institution ?? "Sem instituição"}</p>
                  {account.type === "credit_card" ? (
                    <div className="mt-4 grid grid-cols-3 gap-2 text-sm text-[#c4d0c8]">
            ...
            <EmptyState title="Nenhuma conta cadastrada" description="Cadastre cartões, Pix, débito, banco ou dinheiro para começar a registrar transações." />
          )}
        </section>
 
        <section className="glass-panel rounded-[1.7rem] p-5">
          <h2 className="text-xl font-semibold text-white">Nova conta</h2>
          <p className="mt-1 text-sm text-[#96a59b]">Para cartão de crédito, preencha limite, fechamento e vencimento.</p>
